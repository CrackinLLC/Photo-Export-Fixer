...
def auto_download_exiftool():
    ...
    try:
        ...
        # Find the executable - it may be in a subdirectory and named exiftool(-k).exe
        exe_found = False
        for root, dirs, files in os.walk(tools_dir):
            for f in files:
                if f.startswith("exiftool") and f.endswith(".exe"):
                    src_path = os.path.join(root, f)
                    dest_path = os.path.join(tools_dir, "exiftool.exe")
                    import shutil
...
def find_file(jsondata, file_index, suffixes): # get full path to the file using O(1) dictionary lookup
...
def main(path, suffixes, destination, write_exif=True): # main function, where everything is being done
    ...
    if write_exif and EXIFTOOL_AVAILABLE:
        ...
        print("Note: pyexiftool not installed. Run 'pip install pyexiftool' to enable metadata writing.")
...
def parse(description): # start point of the program, where variable "path" is being created
    ...
    parser.add_argument("-e", "--extend", help="Extend metadata on already-processed files (looks for <source>_pefProcessed)", action="store_true")
    parser.add_argument("--no-exif", help="Skip EXIF metadata writing (faster, timestamps only)", action="store_true")
    parser.add_argument("--dry-run", help="Show what would be done without making changes", action="store_true")
    ...
    if args.extend:
        ...
        if not exists(extend_path):
            ...
            print("Make sure you've run the main processing first, or specify --destination if you used a custom output location.")
            return
