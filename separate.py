...
def main():
    ...
    if not pef.exists(source) or not pef.exists(destination):
        print("Source or destination path does not exist.")
        return
    
    # Get lists of JSON files and other files from the source directory
    jsons, files = pef.get_file_names(source)
    
    # If there are files to process, create a new output directory
    if jsons or files:
        destination = pef.checkout_dir(os.path.join(destination, 'SeparateOutput'), onlynew = True)
    else:
    ...
    if jsons:
        pef.checkout_dir(os.path.join(destination, 'jsons'))
    ...
    if files:
        pef.checkout_dir(os.path.join(destination, 'files'))
    for file in files:
